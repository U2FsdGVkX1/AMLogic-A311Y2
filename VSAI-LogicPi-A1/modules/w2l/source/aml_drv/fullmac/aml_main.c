...
unsigned char get_host_cmd_index(unsigned char msg_type)
{
    ...
    } else if (msg_type == MM_SUB_SET_MCC_RATIO) {
        return 5;
...
char g_cmd_index = (HOST_CMD_COUNT + 1);
...
int aml_cfg80211_init(struct aml_plat *aml_plat, void **platform_data)
{
    ...
    if ((aml_partner_cust == ROKU_DONGLE_VER) || (aml_partner_cust == ROKU_TV_VER))
        aml_send_scan_param_req(aml_hw, ROKU_SCAN_ACTIVE_DURATION, ROKU_SCAN_PASSIVE_DURATION, ROKU_PROBE_NUM_EACH_SCAN);
