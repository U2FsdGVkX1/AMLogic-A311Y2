...
{
    u8 *ht_ie;
    u8 chan_no = aml_ieee80211_freq_to_chan(target_chdef.chan->center_freq, target_chdef.chan->band);
    /*find HT ie*/
        ...
        switch (AML_VIF_TYPE(vif)) {
            ...
            case NL80211_IFTYPE_P2P_GO:
            {
                ...
                if (!(list_empty(&vif->ap.sta_list)) && scc_use_csa) {
                    ...
                    if (!aml_scc_change_beacon_info(vif->aml_hw->wiphy, vif->ndev, target_chdef)) {
