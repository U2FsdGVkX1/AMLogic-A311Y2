...
int adlak_profile_start(struct adlak_device *padlak, struct adlak_context *context, struct adlak_pm_cfg *pm_cfg,
    ...
    if (layer_start <= pmodel_attr->hw_layer_first) {
        is_layer_start = 1;
    }
    if (1 == pm_cfg->profile_en) {
        ...
        if ((phw_info->rev.all & 0xFFFF) < 0x0300) {
            wpt = ADLAK_PM_BUFFER_SIZE_PERLAYER_V1 * layer_start;
        } else {
            wpt = ADLAK_PM_BUFFER_SIZE_PERLAYER_V2 * layer_start;
        }
        adlak_pm_config(padlak, pm_cfg->profile_iova, pm_cfg->profile_buf_size, wpt);
        if (is_layer_start) {
            context->invoke_time_elapsed_tmp = 0;
        }
    } else if (adlak_enable_save_context_time) {
        pm_stat->start = adlak_os_ktime_get();
        if (is_layer_start == 1) {
            context->invoke_time_elapsed_tmp = 0;
        }
...
int adlak_profile_stop(struct adlak_device *padlak, struct adlak_context *context, struct adlak_pm_cfg *pm_cfg,
    ...
    pmodel_attr =
        adlak_get_model_attr(((struct adlak_task *)padlak->queue.ptask_sch_cur)->context,
                             ((struct adlak_task *)padlak->queue.ptask_sch_cur)->sub_tasks_idx);
    if (layer_end >= pmodel_attr->hw_layer_last) {
        is_layer_end = 1;
    }
    if (1 == pm_cfg->profile_en) {
        ...
        if ((phw_info->rev.all & 0xFFFF) < 0x0300) {
            wpt = ADLAK_PM_BUFFER_SIZE_PERLAYER_V1 * layer_end;
        } else {
            wpt = ADLAK_PM_BUFFER_SIZE_PERLAYER_V2 * layer_end;
        }
        ...
        pmodel_attr =
            adlak_get_model_attr(((struct adlak_task *)padlak->queue.ptask_sch_cur)->context,
                                 ((struct adlak_task *)padlak->queue.ptask_sch_cur)->sub_tasks_idx);
        pm_stat->pm_wpt = pmodel_attr->submit_tasks_num * 256;

#endif
        if (pm_stat->pm_wpt > wpt) {
            AML_LOG_WARN("The PM actual wpt(%d) exceeded the expected wpt(%d) !", pm_stat->pm_wpt,
                         wpt);
        }
